...
namespace Terministrator.Terministrator.BLL
{
    static class MessageTypeToPointSystem
    {
        ...
        public static Entites.MessageTypeToPointSystem Create(Entites.PointSystem pointSystem, Entites.MessageType messageType, float reward)
        {
            return DAL.MessageTypeToPointSystem.Create(new Entites.MessageTypeToPointSystem(pointSystem, messageType, reward));
        }
        ...
        {
            List<Entites.MessageTypeToPointSystem> olds = MessageTypeToPointSystem.GetAll(command.Message.UserToChannel.Channel.PointSystem.ChannelId);
            MessageTypeToPointSystem.DeleteAllFrom(command.Message.UserToChannel.Channel.PointSystem.ChannelId);

            List<Tuple<string, float>> values;
            string errorMessage = ReadArray(command.SplitArguements(';'), out values);

            if (errorMessage == null && values.Count != MessageType.Count() && !values.Exists(x => x.Item1.Equals(UniqueOthers)))
            {
                errorMessage = $"You need to either have {MessageType.Count()} elements or have a default value (only a number, without a name).";
            }
            ...
            if (errorMessage == null)
            {
                ...
                {
                    MessageTypeToPointSystem.Create(command.Message.UserToChannel.Channel.PointSystem, messageType,
            ...
            {
                MessageTypeToPointSystem.DeleteAllFrom(command.Message.UserToChannel.Channel.NamableId);
                foreach (Entites.MessageTypeToPointSystem messageTypeToPointSystem in olds)
                {
                    MessageTypeToPointSystem.Create(messageTypeToPointSystem);
                }
        ...
        private static string ReadArray(string[] amounts, out List<Tuple<string, float>> array)
        {
            ...
            foreach (string amount in amounts)
            {
                ...
                float reward;
                if (!Single.TryParse(splited[0], out reward))
                {
                ...
                if (messageType == null)
                {
                    return $"The message type \"{splited[1]}\" is not recognized. It can only be one of the followings: {MessageType.ToString()}";
                }
